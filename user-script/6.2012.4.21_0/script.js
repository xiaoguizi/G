...
var Douban = /\.douban\.com/i.test(location.href), doubanHelper = {name: '豆瓣助手',id: '49911',version: '6.2012.4.21'};
...
// if(self.location == top.location && Douban){
	...
	bugFix();//修复指定页面的BUG
	$('#db_hp') && ($('#db_hp').title = '版本：V' + doubanHelper.version + ' 运行耗时：' + (new Date() - t1)/1000 + ' 秒');//记录豆瓣助手耗时
	// }else{
	// parent && parent.document.getElementById('fulliframe') && (top.location = self.location);// 对付ISP广告
}else if(/douban\.fm/i.test(location.href)){
	FMdownLoad(); //豆瓣FM下载破解
};
	...
	if(_status){
		...
		db_div.innerHTML = '\
				...
				<div id="功能介绍">\
					...
					<br>　　7、小组贴图：自动识别网页中的图片链接，并将其显示出来。\
					<br>　　8、书影音快速搜索链接，可自定义。\
				...
				<div id="交流增强" style="display:none;">\
						...
						<textarea rows="10" cols="81" id="diyword" style="width:450px;"></textarea><br>\
				...
				<div id="相册增强" style="display:none;">\
						...
						<br>  从此克服豆瓣看图的种种不便，看图不再是麻烦。\
						<br>  此功能配合豆瓣助手自带的加载全部页面功能一起使用，效果更佳。\
						<br>  （后续版本将提供更丰富的配置选项）</p>\
						<br>\
						<br>\
				</div>\
				<div id="页面加载" style="display:none;">\
						...
						<br>  若开启“自动加载下一页”功能，浏览到网页底部时，豆瓣助手会自动加载下一页。</p>\
						<br>\
						<br>\
						<br>\
						<br>\
				...
				<div id="快速搜索" style="display:none;">\
						...
						<textarea rows="10" cols="81" id="otherEngines" style="width:450px;"></textarea>\
				...
				<div id="其他功能" style="display:none;">\
						...
						<br>\
						<label><input type="checkbox" id="supportHelper" />通过在卓越、淘宝商城上买书支持豆瓣助手</label>\
						<br>\
						<br>\
						<label><input type="checkbox" id="friendsTool" />开启友邻工具条</label>\
						<br>\
...
function sw_set(ch){
	...
	if (ch){
		...
		db_write('supportHelper'); //通过在卓越、淘宝商城上买书支持豆瓣助手
		...
		db_read('supportHelper',true);//通过在卓越、淘宝商城上买书支持豆瓣助手
	...
	if (/album|photos(?!\/photo)/i.test(location.href) && photitle){
		...
		if(!$('#switchHighslide')){//避免加载下一页后重复操作
			...
			photitle.innerHTML += '<br/>'
				+ (GM_getValue('highslide', false)
					...
					: '<a href="javascript:" id="switchHighslide" class="btn-donate fleft" title="点击开启高阶浏览模式，享受浏览图片的乐趣 by豆瓣助手">开启浏览模式</a>'
					)
		...
		if(GM_getValue('highslide', true)){
			...
			!GM_getValue('highslide_js', false) && GM_xmlhttpRequest({
				method: 'GET',
				url: 'http://js-addon.googlecode.com/files/highslide.js',
				onload: function(xhr){
					if(xhr.status == 200){
			...
			!GM_getValue('highslide_css', false) && GM_xmlhttpRequest({
				method: 'GET',
				url: 'http://js-addon.googlecode.com/files/highslide.css',
				onload: function(xhr){
					if(xhr.status == 200){
...
// function starBackup(){
	...
	// if(user){
		...
		// span.innerHTML = '<a href="javascript:void(0);" id="starBackup" class="btn-donate" title="点击后备份日记  by豆瓣助手">[资料备份]</a>';
...
function addSearchBar() {
	if ($('#mainpic') && GM_getValue('search_bar', true) && !$('.s_bar')[0]){
		...
		var keyword = $$('h1 span')[0].textContent;//取得书影音标题
...
function hideMSG(){
	...
	if(h2 && /最近小组话题/.test(h2.innerHTML) && !$('#_toggle')){
		$('#header').innerHTML += '<a id="_toggle" style="font-size:12px;position:absolute;top:114px;left:475px;z-index:1;color:#060;">&gt;显示/隐藏小组介绍&lt;</a>';
...
function autoElevator(){
	var reNums = xpath('.//table[@class="list-b" or @class="olt"]/tbody/tr[not(@dbmark)]/td[last()-1]');
	if( GM_getValue('autoElevator',true) && reNums.snapshotItem(0) &&/回应/.test(reNums.snapshotItem(0).textContent + reNums.snapshotItem(1).textContent) ){//论坛表格第一行没有“回应”
		GM_addStyle('.pageNum{float:right;}');
		var reply = xpath('.//table[@class="list-b" or @class="olt"]/tbody/tr[not(@dbmark)]/td[1]');
		for(var i=0,j=reNums.snapshotLength; i<j; i++){
			var title = reply.snapshotItem(i), numWord = reNums.snapshotItem(i).textContent.replace(/[\s\D]*/g,''), num = Number(numWord), lastPage, href, linkCache = '<span class="pageNum">(';
			title.parentNode.setAttribute('dbmark', '1');//已处理标记，加载下一页后不重复处理
			if(!isNaN(num) && num>100){
				lastPage = numWord.slice(0, -2);
				href = title.getElementsByTagName('a')[0].href;
				// num > 400 && (linkCache += ' <a href="'+ href +'?start='+ (lastPage-3) +'00" title="倒数第四页">'+ (lastPage-2) +'</a> |');
				// num > 300 && (linkCache += ' <a href="'+ href +'?start='+ (lastPage-2) +'00" title="倒数第三页">'+ (lastPage-1) +'</a> |');
...
function exGroup(){
	...
	if ( (ct || jv || re) && postArea ){
		...
		//工具条CSS & HTML
		GM_addStyle('a.ln-report{position:static;} .ctrl_tool a{margin-left:0px;}');//修复“举报广告”阻挡工具条的问题等样式问题
		GM_addStyle(GM_getValue('auto_hide', false)?'.ctrl_tool{display:none;} li:hover .ctrl_tool{display:block;}':'');
		var re_s = re?'<a name="db_re" href="javascript:" title="回复该用户发言 by豆瓣助手" >回</a>\
					<a name="db_qt" href="javascript:" title="引用该用户发言" >引</a> ':'';
		var re_l = re?'&gt;<a name="db_re" href="javascript:" title="回复该用户发言 by豆瓣助手" >回复</a>&nbsp;&nbsp;\
		...
		for (var i=0,l=replys.snapshotLength; i<l; i++){
			...
			var bar = replyI.querySelector('div.group_banned, td:last-child, .operation_div');
			bar && (bar.className += ' admin-lnks');
			if (clibtn && bar){
				var span = document.createElement('span');
				span.className = /\/group\/topic\//i.test(location.href) ? 'ctrl_tool' : 'fright ctrl_tool';
		...
		var LZbarlink = $$('#db-usr-profile a, .post-info>.from>a, h3>.pl20>a, .piir a, .wr a, .aside>.p12>a, .aside>.pl2>a')[0];// .aside>.p12>a, .aside>.pl2>a 豆瓣日记页面的楼主链接
		if (jv && LZbar && LZbarlink && !$('#autolift')){
			...
			toptool.innerHTML = '\
				...
				&gt;<a href="javascript:" id="autolift" title="点击自动生成电梯链接" >生成电梯</a>&nbsp;&nbsp;';
...
function sortGroup(){
	if(/group\/mine/i.test(location.href) && GM_getValue('sortGroup', true)){
		GM_addStyle('h1 {width:590px;} h1 span{font-weight:normal;} #bydbhp {visibility:hidden;color:#BBB;font-size:12px;} h1:hover #bydbhp {visibility:visible;}\
			...
			.article dl:hover {-moz-outline:1px solid #a6d098;outline:1px solid #a6d098;-moz-outline-radius:3px;-webkit-outline-radius:3px;background:#efe;}\
			fieldset {-moz-border-radius:5px;-webkit-border-radius:5px;}\
			fieldset div a{-moz-border-radius:10px;-webkit-border-radius:10px;background:#FFF;border:1px solid #BBB;color:#BBB;display:block;width:17px;height:17px;bottom:20px;left:568px;position:relative;text-align:center;}\
		...
		// 增加分类区
		$('.article')[0].insertBefore(document.createElement('div'), $$('.article h2')[0]);
		...
		$$('h1')[0].innerHTML += '\<span class="gact fright">\
			...
			<span id="bydbhp" class="gact fright">分类完毕后请别忘了点击保存(by豆瓣助手)&nbsp;&nbsp;&nbsp;</span>';
...
function FMdownLoad(){
	if($('#fm-player')){
		...
		downDiv.innerHTML = '<span style="float:left;"><a href="javascript:;" id="palyPreSong" title="播放上一首　　by豆瓣助手">« 上一首</a></span>\
			<span style="float:right;" title="Hack by 豆瓣助手">下载：<a href="javascript:;" id="songDownLink" title="请使用下载工具，或右键另存为下载　　by豆瓣助手" target="_blank">暂无下载</a></span>';
		$('#fm-player').appendChild(downDiv);
		GM_addStyle('#fm-player span{color:#999;} #fm-player span a{color:#4e5553;} #fm-player span a:hover{color:#4e5553;background:#9dd6c5}');
		
		// var date = JSON.parse(o);
		location.href = 'javascript:void(setTimeout(\'(function(){\
...
function settle(){
	if ($('.aside')[0] && GM_getValue('settler', true)){
		...
		settler.innerHTML = '\
			...
			<a href="javascript:;" title="置底，快捷键为：'+GM_getValue('hk_btm', 'S')+'" onclick="scrollTo(0,99999999);">&nbsp;Btm↓&nbsp;</a>\
		...
		addHotKey(GM_getValue('hk_btm', 'S'),function(){scrollTo(0,99999999)});
...
// 顶部滑动导航条
function slideBar(){
	if($('.top-nav-items')[0] && GM_getValue('slideBar', true)){
		var name = getUserName();
		
		//添加我的豆瓣
		if(name){
			var myDouban = document.createElement('li');
			myDouban.innerHTML = '<a href="http://www.douban.com/mine/" title="by 豆瓣助手">我的豆瓣</a>';
			$$('.top-nav-items ul')[0].insertBefore(myDouban, $$('.top-nav-items ul li')[0]);
		};
		
		//添加豆瓣说到导航条 因为遮挡搜索按钮暂时去除
		// if($('.top-nav-more-items')[0]){
			// $$('.top-nav-more-items ul')[0].innerHTML += '<li><a href="http://shuo.douban.com/" target="_blank" title="by 豆瓣助手">豆瓣说</a></li>';
		...
		';
		$('.top-nav-items')[0].appendChild(subNavItems);
		
		$$('.top-nav-items ul')[0].addEventListener('mouseover', function(e){
			if(e.target.href){
				setStyles($$('#top-nav-items ul'), 'display:none;');
...
function musicLink(){
	...
	if(list[0] && GM_getValue('musicLink', true)){
		var getMusicLink = function(playlist){
			...
			multiDownload && (multiDownload.innerHTML = '<a href="javascript:;" title="批量下载  by豆瓣助手" name="" onclick="var txt=document.createElement(\'textarea\');txt.style.cssText=\'width:572px;height:100px;font-size:9px;\';this.parentNode.parentNode.parentNode.appendChild(txt);txt.value=this.name;">批量下载</a>');
			for(var l,i=0,j=downLinks.snapshotLength; i<j; i++){
				l = downLinks.snapshotItem(i);
				l.innerHTML = '<a href="'+ atob(song_records[i]['url'])+'" title="右键另存为下载：'+decodeURIComponent(song_records[i]['name'])+'   by豆瓣助手"><img src="http://t.douban.com/pics/download.gif"/></a>';
	...
	if(/douban\.com\/(?:[^\/]+\/[^\/]+|widget)\/board/i.test(location.href) && GM_getValue('exBoard', true)){
			...
			mainArea.innerHTML = '\
				<div id="board">\
					<form id="fboard" style="margin-bottom: 12px;" method="post" name="bpform" action='+ action +'>\
						...
						<span class="bn-flat"><input type="submit" value=" 留言 " name="bp_submit"></span><span id="bydbhp">by豆瓣助手</span>\
		...
		for (var i=0,r=$$('.mbtrdot, .update-item .author'),l=r.length; i<l; i++){
			if(!/dbmark/.test(r[i].className)){
				r[i].innerHTML += '<span class="gact"><a title="留言到对方的留言板 by豆瓣助手" onclick="var link=this.parentNode.parentNode.getElementsByTagName(\'a\')[0];var form=$(\'#fboard\');form.attr(\'action\',link.href+\'board\');form.find(\'input:visible\').val(\'留言到\'+link.innerHTML+\'的留言板\');form.find(\'textarea\').focus();form.find(\'a\').attr(\'style\',\'\');" href=#>回复</a></span>';
	...
	if(paginator && GM_getValue('addPageLoader', true)){
		...
		if($('#nextPageLoder')){
			...
			$('#allPageLoder').addEventListener('click', function(){
				if(!!$$('.paginator>span.break')[0]){//超过10页
					if(!window.loadPageConfirm){//本页未提示过
						...
						confirm('检测到分页较多，可能耗时较长，是否加载？\n（当前设定为每次加载'+GM_getValue('loadPageNum', 10)+'页，可点击“豆瓣助手”设置）') && loadPage(false, runHelper);
					}else{//本页已提示过
			...
			GM_getValue('autoLoadPage', true) && document.body.addEventListener(navigator.userAgent.indexOf('Chrome')>0 ? 'mousewheel' : 'DOMMouseScroll', function(){//firefox使用DOMMouseScroll chrome使用mousewheel
				...
				if(pageLoaderY - window.scrollY <= window.innerHeight && $$('.paginator:not(#secPaginator)>.next>a')[0] && window.autoLoading !== true){// 滚动到加载控件所在位置，存在下一页，且不在加载中，则自动加载
